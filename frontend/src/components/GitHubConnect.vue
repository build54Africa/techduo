<template>
      ...
      <form @submit.prevent="connectRepo" class="space-y-4">
        ...
        <div class="flex justify-end space-x-3">
          <button 
            ...
            class="px-4 py-2 text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 disabled:opacity-50 transition-colors"
          >
            Search
          </button>
          <button 
  ...
  const connectRepo = async () => {
    try {
      const result = await prStore.connectRepository(form.owner, form.repoName)
